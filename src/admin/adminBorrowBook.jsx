...
function AdminBorrowBook({ onBookReturned }) {
    ...
    const handleReturnBook = async (recordId, bookId, borrowerUsername) => {
        try {
            ...
            // Reset user's request count in backend
            await axios.patch(
                `https://appdev-integrative-28.onrender.com/api/library/users/${borrowerUsername}/decrement-request/`,
                {},
                { headers: { 'Authorization': `Token ${authToken}` } }
            );
    ...
    return (
        <div className='dashboard'>
            ...
            <section className='searchBooks'>
                ...
                />
                <section className='totalBorrowed'>
                    <p className='label'>Borrowed Books</p>
                    <p className='label' style={{ fontSize: '100px' }}>{borrowedBooksCount}</p>
                </section>
            </section>
